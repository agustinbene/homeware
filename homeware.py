...
@app.route('/test')
def test():
    publish.single("test", "payload", hostname="192.168.19.92")
    return 'Load'
    ...
    for rule in rules:
        ...
        if verified == ammountTriggers:
            for target in rule['targets']:
                ...
                publish.single("device/"+target['id'], json.dumps(data['status'][target['id']]), hostname="192.168.19.92")
...
def on_message(client, userdata, msg):
    ...
    if intent == 'execute':
        ...
        writeJSON(data)
        publish.single("device/"+id, json.dumps(data['status'][id]), hostname="192.168.19.92")
    elif intent == 'rules':
        data = readJSON();
        data['status'][id][param] = value;
        writeJSON(data)
...
def mqttReader():
    ...
    client.connect("192.168.19.92", 1883, 60)
    client.loop_forever()
